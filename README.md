...
Either path does the same thing: clones (or updates) the repo when needed, generates a `PROJECT_ID` like `orkestra-a1b2c3` if you omit one, writes `.env`, builds and starts Docker, waits for health, installs the operator skill to `~/.agents/skills/orkestra/`, and registers MCP for selected AI tools (Claude Code, Codex, Cursor, Copilot, Windsurf, Zed, Continue.dev). Interactive installs prompt for tools after detection; non-interactive installs configure all detected tools unless `ORKESTRA_TOOLS` is set. The installer is idempotent - re-running it updates config in place.
...
**Manual / no Docker** (Go 1.26+ required, useful for local dev):
...
No Docker required for development. You need Go 1.26+.
...
| ----------------- | ------------------- | -------------------------------------------------------- |
| `PROJECT_ID`      | _(required)_        | Ticket ID prefix (`myapp`) and scope filter; generated by installer, must be set manually for direct `go run` / Docker use |
